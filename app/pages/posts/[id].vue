<template>
  <article class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div v-if="status === 'pending'"
      ...
      <UIcon name="i-lucide-loader-2" class="size-8 animate-spin mb-3 text-amber-500" />
    ...
    <div v-else-if="error || !post"
      ...
      <UIcon name="i-lucide-alert-triangle" class="size-8 mx-auto mb-2" />
    ...
    <template v-else>
      ...
      <div
        ...
        <div class="flex items-center gap-3">
          <UAvatar :src="post.author?.avatarUrl ?? `https://i.pravatar.cc/100?img=${post.author?.id ?? 1}`"
            :alt="post.author?.name ?? 'User'" size="md" class="ring-1 ring-gray-200 dark:ring-gray-800" />
          <div>
        ...
        <!-- Tombol Aksi (Like & Komentar Counter) -->
        <div class="flex items-center gap-5 text-gray-500 dark:text-gray-400">
          <button class="flex items-center gap-1.5 transition-colors hover:text-amber-500 dark:hover:text-amber-400"
            @click="handleLike" aria-label="Like post">
      ...
      <!-- Hero Image -->
      <div class="mt-8 overflow-hidden rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
        <NuxtImg :src="post.imageUrl ?? `https://picsum.photos/seed/${post.id}/1200/700`" :alt="post.title" width="1200"
          height="700" class="aspect-[16/9] w-full object-cover" />
      </div>

      <!-- Body / Isi Artikel -->
      <div
        class="prose prose-gray dark:prose-invert mt-10 max-w-none text-gray-700 dark:text-gray-300 leading-relaxed text-base sm:text-lg">
      ...
      <section id="comments" class="mt-16 border-t border-gray-100 pt-10 dark:border-gray-800">
        ...
        <div class="space-y-6">
          <div v-for="comment in post.comments" :key="comment.id" class="flex gap-3.5 text-sm">
            <UAvatar :src="comment.author?.avatarUrl ?? `https://i.pravatar.cc/100?img=${comment.author?.id ?? 1}`"
              :alt="comment.author?.name ?? 'User'" size="sm" class="ring-1 ring-gray-200 dark:ring-gray-800" />
            ...
            <div class="flex-1">
              ...
              <div v-if="editingCommentId === comment.id" class="mt-2 space-y-2">
                ...
                <div class="flex items-center gap-2">
                  <UButton size="xs" :loading="editPending"
              ...
              <template v-else>
                ...
                <div v-if="comment.author?.id === CURRENT_USER_ID" class="mt-2 flex gap-3 text-xs text-gray-400">
                  <button class="transition-colors hover:text-gray-700 dark:hover:text-gray-200"
