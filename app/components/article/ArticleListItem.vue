<template>
  <NuxtLink :to="`/posts/${post.id}`" class="group flex flex-col md:flex-row gap-5 md:gap-8 py-8 border-b border-gray-100 dark:border-gray-800 items-start transition-colors">
    ...
    <div class="flex flex-col flex-1 min-w-0 order-2 md:order-1 w-full">
      ...
      <h2 class="text-xl md:text-2xl font-extrabold text-gray-900 dark:text-white leading-tight mb-2 tracking-tight transition-colors group-hover:text-gray-600 dark:group-hover:text-gray-300">
        {{ post.title }}
      </h2>
      ...
      <div class="flex items-center justify-between mt-auto">
        ...
        <div v-else></div> <div class="flex items-center gap-5 text-gray-500 dark:text-gray-400">
...
>
  ...
  <span 
    ...
    :class="isLiked && 'text-amber-500 dark:text-amber-400'"
  >
    {{ likeCount }}
  </span>
</button>
